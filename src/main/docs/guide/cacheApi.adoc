...
<3> Emulate an expensive operation by sleeping for 15 seconds.
<4> The return value is cached with name `headlines` for the supplied `month`. The method invocation is not skipped.
<5> Indicates the invocation that the method should cause the invalidation of the cache `headlines` for the supplied `month`.